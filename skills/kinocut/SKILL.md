...
3. Prefer previews or dry-run manifests before expensive or destructive exports:
   ...
   - For supplied ambient layers, use version3 with ordered layer/source bindings and explicit pad or crossfaded loop fill; see `docs/SOUND_LAYER_REQUESTS.md`. Inspect `layers_sha256` and measured frame evidence. Bed ducking affects only the separate bed; layer ducking and scene schedules remain unsupported. Listen to loop seams before accepting the assembly.
