...
3. Prefer previews or dry-run manifests before expensive or destructive exports:
   ...
   - For track/bus gain, pan and mute/solo, use version2 with explicit cue-track bindings; see `docs/SOUND_ROUTING_REQUESTS.md`. V2/V3 support gain/pan envelopes (`docs/SOUND_AUTOMATION_REQUESTS.md`) and finite pre/post-fader sends (`docs/SOUND_SEND_REQUESTS.md`). Inspect automation/send hashes and full graph/source-window evidence. Send cycles, unsupported parameters, general sidechains and effects are rejected.
   - For supplied ambient layers, use version3 with ordered layer/source bindings and explicit pad or crossfaded loop fill; see `docs/SOUND_LAYER_REQUESTS.md`. Optional `layer_ducking` uses one declared source bus and targets explicit ambience layers; inspect `layer_ducking_sha256`, completed releases and truncated recovery alongside `layers_sha256`. Bed ducking affects only the separate bed. Scene schedules and general routing sidechains remain unsupported. Listen to seams and gain recovery before accepting the assembly.
