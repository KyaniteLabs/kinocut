"""``kinocut_sound`` — standalone audio-play production contracts.

The foundation leaf (S1) of the Sonic World audio-play production design.
This package ships **inside** the Kinocut repository but is fully usable
without importing any ``kinocut`` runtime module: it re-implements the
canonical-record pattern (frozen Pydantic models, fail-closed typed ids,
sorted-key JSON digests) so the sidecar boundary stays clean.

Public surface (this leaf):

* :class:`SoundPlan` and supporting provenance refs.
* :class:`Timeline`, :class:`Cue`, :class:`CueKind`.
* :class:`AudioFormat` and closed enums for layout/sample/time-base/dither.
* :class:`Routing`, :class:`Track`, :class:`Bus`, sends, sidechains, automation.
* :class:`Line`, :class:`ProfileRef`, :class:`Prosody`, :class:`Emotion`.
* :class:`DeliveryPolicy`, :class:`DeliveryPreset`, loudness and stem policy.
* :class:`ConsentGrant`, :class:`ConsentScope`, :class:`CloudEgressGrant`.
* :class:`AdapterDescriptor`, :class:`CapabilityResult`, :class:`CostDisclosure`.
* :class:`SoundReceipt`, :class:`SoundReceiptSection`, :class:`OrderedInput`.
* :class:`RenderFingerprint`, :class:`DeterminismClass`.
* :class:`SoundContractError` and the stable contract error codes.

Numerical defaults (loudness tolerance, true-peak ceilings, latency residual,
gap tolerance) live in :mod:`kinocut_sound.defaults`; resource ceilings
(stem-recombination tolerance, max loudness tolerance, latency residual) live
in :mod:`kinocut_sound.limits`; and closed validation regexes/sets live in
:mod:`kinocut_sound.validation`. Adapter authors import these directly rather
than reaching into owning modules.
"""

# New leaves added in this integration (S5 voice, S7 post, S8 world) keep
# their public exports in their own subpackages. The top-level surface below
# re-exports the key contracts so adapter authors and plans can import from
# ``kinocut_sound`` directly while the sidecar boundary stays clean.

from __future__ import annotations

from kinocut_sound._canonical import (
    BoundedCode as BoundedCode,
    FrozenModel as FrozenModel,
    RecordBase as RecordBase,
    Sha256 as Sha256,
    canonical_digest as canonical_digest,
    canonical_record_id as canonical_record_id,
    location_violation as location_violation,
)
from kinocut_sound._errors import (
    INVALID_RECORD as INVALID_RECORD,
    UNSAFE_LOCATION as UNSAFE_LOCATION,
    UNKNOWN_RECORD_FIELD as UNKNOWN_RECORD_FIELD,
    SoundContractError as SoundContractError,
    contract_error as contract_error,
)
from kinocut_sound.defaults import (
    DEFAULT_MIX_CHANNEL_COUNT,
    DEFAULT_MIX_SAMPLE_RATE_HZ,
    DEFAULT_MIX_DUCK_ATTENUATION_DB,
    DEFAULT_MIX_DUCK_ATTACK_MS,
    DEFAULT_MIX_DUCK_RELEASE_MS,
    DEFAULT_MIX_DUCK_THRESHOLD,
    DEFAULT_ASR_TOTAL_TIMEOUT_SECONDS,
    DEFAULT_ASR_PROBE_TIMEOUT_SECONDS,
    DEFAULT_ASR_THREADS,
    DEFAULT_ASR_SAMPLE_RATE_HZ,
    DEFAULT_ASR_DECODE,
    DEFAULT_MASTER_TOTAL_TIMEOUT_SECONDS,
    DEFAULT_MASTER_PEAK_MARGIN_DB,
    DEFAULT_MASTER_LRA_LU,
    DEFAULT_MASTER_LIMITER_ATTACK_MS,
    DEFAULT_MASTER_LIMITER_RELEASE_MS,
    DEFAULT_LOUDNESS_METER_TIMEOUT_SECONDS,
    DEFAULT_LOUDNESS_VERSION_TIMEOUT_SECONDS,
    DEFAULT_LOUDNESS_DEMO_DURATION_SECONDS,
    DEFAULT_DUB_AMPLITUDE as DEFAULT_DUB_AMPLITUDE,
    DEFAULT_DUB_SAMPLE_RATE_HZ as DEFAULT_DUB_SAMPLE_RATE_HZ,
    DEFAULT_SPATIAL_DISTANCE_PCT as DEFAULT_SPATIAL_DISTANCE_PCT,
    DEFAULT_SPATIAL_CROSSOVER_HZ as DEFAULT_SPATIAL_CROSSOVER_HZ,
    DEFAULT_SPATIAL_ROLLOFF_DB as DEFAULT_SPATIAL_ROLLOFF_DB,
    DEFAULT_SPATIAL_DISTANCE_GAIN_DB as DEFAULT_SPATIAL_DISTANCE_GAIN_DB,
    DEFAULT_SPATIAL_DISTANCE_Q as DEFAULT_SPATIAL_DISTANCE_Q,
    DEFAULT_DUB_SPATIAL_THREADS as DEFAULT_DUB_SPATIAL_THREADS,
    DEFAULT_DUB_TOTAL_TIMEOUT_SECONDS as DEFAULT_DUB_TOTAL_TIMEOUT_SECONDS,
    DEFAULT_DUB_UTTERANCE_TIMEOUT_SECONDS as DEFAULT_DUB_UTTERANCE_TIMEOUT_SECONDS,
    DEFAULT_DUB_WORDS_PER_MINUTE as DEFAULT_DUB_WORDS_PER_MINUTE,
    DEFAULT_ADAPTER_TIMEOUT_SECONDS as DEFAULT_ADAPTER_TIMEOUT_SECONDS,
    DEFAULT_BROADCAST_TRUE_PEAK_DBTP as DEFAULT_BROADCAST_TRUE_PEAK_DBTP,
    DEFAULT_BUS_GAIN_DB as DEFAULT_BUS_GAIN_DB,
    DEFAULT_GAP_TOLERANCE_SECONDS as DEFAULT_GAP_TOLERANCE_SECONDS,
    DEFAULT_LATENCY_RESIDUAL_SAMPLES as DEFAULT_LATENCY_RESIDUAL_SAMPLES,
    DEFAULT_LOUDNESS_TOLERANCE_LU as DEFAULT_LOUDNESS_TOLERANCE_LU,
    DEFAULT_PAN_POSITION as DEFAULT_PAN_POSITION,
    DEFAULT_PROSODY_EMPHASIS as DEFAULT_PROSODY_EMPHASIS,
    DEFAULT_PROSODY_PITCH_SEMITONES as DEFAULT_PROSODY_PITCH_SEMITONES,
    DEFAULT_PROSODY_RATE as DEFAULT_PROSODY_RATE,
    DEFAULT_PROSODY_VOLUME_DB as DEFAULT_PROSODY_VOLUME_DB,
    DEFAULT_PROVIDER_CONNECT_TIMEOUT_SECONDS as DEFAULT_PROVIDER_CONNECT_TIMEOUT_SECONDS,
    DEFAULT_PROVIDER_MAX_CONCURRENCY as DEFAULT_PROVIDER_MAX_CONCURRENCY,
    DEFAULT_PROVIDER_MAX_DURATION_SECONDS as DEFAULT_PROVIDER_MAX_DURATION_SECONDS,
    DEFAULT_PROVIDER_MAX_INPUT_BYTES as DEFAULT_PROVIDER_MAX_INPUT_BYTES,
    DEFAULT_PROVIDER_MAX_OUTPUT_BYTES as DEFAULT_PROVIDER_MAX_OUTPUT_BYTES,
    DEFAULT_PROVIDER_MAX_RETRIES as DEFAULT_PROVIDER_MAX_RETRIES,
    DEFAULT_PROVIDER_RATE_LIMIT_PER_MINUTE as DEFAULT_PROVIDER_RATE_LIMIT_PER_MINUTE,
    DEFAULT_PROVIDER_READ_TIMEOUT_SECONDS as DEFAULT_PROVIDER_READ_TIMEOUT_SECONDS,
    DEFAULT_PROVIDER_TOTAL_TIMEOUT_SECONDS as DEFAULT_PROVIDER_TOTAL_TIMEOUT_SECONDS,
    DEFAULT_PUBLIC_MIX_TIMEOUT_SECONDS as DEFAULT_PUBLIC_MIX_TIMEOUT_SECONDS,
    DEFAULT_MIX_CONVERSION_THREADS as DEFAULT_MIX_CONVERSION_THREADS,
    DEFAULT_MIX_CONVERSION_PRECISION as DEFAULT_MIX_CONVERSION_PRECISION,
    DEFAULT_MIX_CONVERSION_CUTOFF as DEFAULT_MIX_CONVERSION_CUTOFF,
    DEFAULT_MIX_CONVERSION_CHEBY as DEFAULT_MIX_CONVERSION_CHEBY,
    DEFAULT_MIX_CONVERSION_DITHER_METHOD as DEFAULT_MIX_CONVERSION_DITHER_METHOD,
    DEFAULT_PUBLIC_MIX_STEMS as DEFAULT_PUBLIC_MIX_STEMS,
    DEFAULT_SEND_GAIN_DB as DEFAULT_SEND_GAIN_DB,
    DEFAULT_SEND_POST_FADER,
    DEFAULT_LAYER_DUCKING_ATTACK_MS,
    DEFAULT_LAYER_DUCKING_ATTENUATION_DB,
    DEFAULT_LAYER_DUCKING_RECOVERY_MS,
    DEFAULT_LAYER_DUCKING_RELEASE_MS,
    DEFAULT_STREAM_PODCAST_TRUE_PEAK_DBTP as DEFAULT_STREAM_PODCAST_TRUE_PEAK_DBTP,
    DEFAULT_TAIL_SECONDS as DEFAULT_TAIL_SECONDS,
)
from kinocut_sound.limits import (
    MAX_MIX_RECEIPT_BYTES,
    MAX_MIX_ROUTING_TRACKS,
    MAX_MIX_ROUTING_BUSES,
    MAX_MIX_ROUTING_BINDINGS,
    MAX_MIX_ROUTING_RECEIPT_BYTES,
    MIX_ROUTING_SOURCE_MEMORY_MULTIPLIER,
    MAX_AMBIENT_EXTRA_REPEATS,
    MAX_AMBIENT_LAYERS,
    MAX_LAYER_DUCKING_SAMPLE_VISITS,
    MAX_MIX_AUTOMATION_ENVELOPES,
    MAX_MIX_AUTOMATION_POINTS,
    MAX_MIX_AUTOMATION_POINTS_PER_ENVELOPE,
    MAX_MIX_AUTOMATION_WORK_UNITS,
    MIX_AUTOMATION_METADATA_BYTES,
    MAX_MIX_SENDS,
    MAX_MIX_SEND_RECEIPT_BYTES,
    MIX_SEND_METADATA_BYTES,
    MAX_MIX_SEND_WORK_UNITS,
    MAX_MIX_ROUTED_FEATURE_WORK_UNITS,
    MAX_MIX_SIDECHAINS,
    MAX_MIX_SIDECHAIN_RECEIPT_BYTES,
    MIX_SIDECHAIN_METADATA_BYTES,
    MAX_MIX_SIDECHAIN_WORK_UNITS,
    MAX_MIX_LAYER_RECEIPT_BYTES,
    MIX_LAYER_WORK_MEMORY_MULTIPLIER,
    MAX_ASR_DURATION_SECONDS,
    MAX_ASR_SAMPLE_RATE_HZ,
    MAX_ASR_TEXT_BYTES,
    MAX_ASR_WORDS,
    MAX_ASR_SEGMENTS,
    MAX_ASR_OUTPUT_BYTES,
    MAX_ASR_MODEL_BYTES,
    MAX_ASR_STAGING_BYTES,
    ASR_MEMORY_ESTIMATE_BYTES,
    ASR_COPY_CHUNK_BYTES,
    MAX_ASR_LAUNCHER_BYTES,
    MAX_MASTER_WAV_HEADER_BYTES,
    MASTER_MEMORY_MULTIPLIER,
    MAX_LOUDNESS_DIAGNOSTIC_BYTES,
    LOUDNESS_DIAGNOSTIC_CHUNK_BYTES,
    MIN_LOUDNESS_MATERIAL_SECONDS,
    MIN_MEASURABLE_INTEGRATED_LUFS,
    DUB_ASSEMBLY_MEMORY_MULTIPLIER as DUB_ASSEMBLY_MEMORY_MULTIPLIER,
    DUB_SPATIAL_NATIVE_MEMORY_BYTES as DUB_SPATIAL_NATIVE_MEMORY_BYTES,
    DUB_SPATIAL_METADATA_BYTES as DUB_SPATIAL_METADATA_BYTES,
    DUB_SPATIAL_IO_BYTES as DUB_SPATIAL_IO_BYTES,
    MAX_DUB_CAPTION_BYTES as MAX_DUB_CAPTION_BYTES,
    MAX_DUB_CUES as MAX_DUB_CUES,
    MAX_DUB_CUE_CHARS as MAX_DUB_CUE_CHARS,
    MAX_DUB_MEMORY_BYTES as MAX_DUB_MEMORY_BYTES,
    MAX_DUB_TIMELINE_SECONDS as MAX_DUB_TIMELINE_SECONDS,
    MAX_DUB_TOTAL_CHARS as MAX_DUB_TOTAL_CHARS,
    MAX_DUB_WAV_BYTES as MAX_DUB_WAV_BYTES,
    MAX_MIX_DURATION_SECONDS as MAX_MIX_DURATION_SECONDS,
    MAX_MIX_INPUT_BYTES as MAX_MIX_INPUT_BYTES,
    MAX_MIX_JSON_DEPTH as MAX_MIX_JSON_DEPTH,
    MAX_MIX_MEMORY_BYTES as MAX_MIX_MEMORY_BYTES,
    MAX_MIX_REQUEST_BYTES as MAX_MIX_REQUEST_BYTES,
    MAX_MIX_SAMPLE_RATE_HZ as MAX_MIX_SAMPLE_RATE_HZ,
    MAX_MIX_STEMS as MAX_MIX_STEMS,
    MAX_MIX_WORKER_MESSAGE_BYTES as MAX_MIX_WORKER_MESSAGE_BYTES,
    MAX_MIX_CONVERTED_BYTES as MAX_MIX_CONVERTED_BYTES,
    MAX_MIX_CONVERSION_MANIFEST_BYTES as MAX_MIX_CONVERSION_MANIFEST_BYTES,
    MAX_MIX_CONVERSION_WORK_UNITS as MAX_MIX_CONVERSION_WORK_UNITS,
    MIX_CONVERSION_STARTUP_WORK_UNITS as MIX_CONVERSION_STARTUP_WORK_UNITS,
    MIX_CONVERSION_NATIVE_MEMORY_BYTES as MIX_CONVERSION_NATIVE_MEMORY_BYTES,
    MIX_CONVERSION_METADATA_BYTES as MIX_CONVERSION_METADATA_BYTES,
    MIX_CONVERSION_IO_BYTES as MIX_CONVERSION_IO_BYTES,
    MAX_MIX_WORKER_DIAGNOSTIC_BYTES as MAX_MIX_WORKER_DIAGNOSTIC_BYTES,
    MIX_WORKER_IO_CHUNK_BYTES as MIX_WORKER_IO_CHUNK_BYTES,
    MIN_MIX_SAMPLE_RATE_HZ as MIN_MIX_SAMPLE_RATE_HZ,
    MIX_SOURCE_MEMORY_MULTIPLIER as MIX_SOURCE_MEMORY_MULTIPLIER,
    MIX_STEM_MEMORY_MULTIPLIER as MIX_STEM_MEMORY_MULTIPLIER,
    MIX_WORK_MEMORY_MULTIPLIER as MIX_WORK_MEMORY_MULTIPLIER,
    MAX_ADAPTER_TIMEOUT_SECONDS as MAX_ADAPTER_TIMEOUT_SECONDS,
    MAX_DUCKING_ATTACK_MS as MAX_DUCKING_ATTACK_MS,
    MAX_ASSEMBLY_CLIPS as MAX_ASSEMBLY_CLIPS,
    MAX_ASSEMBLY_FOLEY_INTENTS as MAX_ASSEMBLY_FOLEY_INTENTS,
    MAX_ASSEMBLY_SILENCE_INTENTS as MAX_ASSEMBLY_SILENCE_INTENTS,
    MAX_ASSEMBLY_TIMELINE_CUES as MAX_ASSEMBLY_TIMELINE_CUES,
    MAX_DUCKING_ATTENUATION_DB as MAX_DUCKING_ATTENUATION_DB,
    MAX_DUCKING_RECOVERY_MS as MAX_DUCKING_RECOVERY_MS,
    MAX_DUCKING_RELEASE_MS as MAX_DUCKING_RELEASE_MS,
    MAX_FINGERPRINT_ITEMS as MAX_FINGERPRINT_ITEMS,
    MAX_FINGERPRINT_VERSION_CHARS as MAX_FINGERPRINT_VERSION_CHARS,
    MAX_GAIN_DB as MAX_GAIN_DB,
    MAX_LATENCY_RESIDUAL_SAMPLES as MAX_LATENCY_RESIDUAL_SAMPLES,
    MAX_LOUDNESS_LUFS as MAX_LOUDNESS_LUFS,
    MAX_LOUDNESS_RANGE_LU as MAX_LOUDNESS_RANGE_LU,
    MAX_LOUDNESS_TOLERANCE_LU as MAX_LOUDNESS_TOLERANCE_LU,
    MAX_NORMALIZED_LEVEL as MAX_NORMALIZED_LEVEL,
    MAX_PAN_POSITION as MAX_PAN_POSITION,
    MAX_PROSODY_PITCH_SEMITONES as MAX_PROSODY_PITCH_SEMITONES,
    MAX_PROSODY_RATE as MAX_PROSODY_RATE,
    MAX_PROSODY_VOLUME_DB as MAX_PROSODY_VOLUME_DB,
    MAX_PROVIDER_CONCURRENCY as MAX_PROVIDER_CONCURRENCY,
    MAX_PROVIDER_CONNECT_TIMEOUT_SECONDS as MAX_PROVIDER_CONNECT_TIMEOUT_SECONDS,
    MAX_PROVIDER_DURATION_SECONDS as MAX_PROVIDER_DURATION_SECONDS,
    MAX_PROVIDER_INPUT_BYTES as MAX_PROVIDER_INPUT_BYTES,
    MAX_PROVIDER_OUTPUT_BYTES as MAX_PROVIDER_OUTPUT_BYTES,
    MAX_PROVIDER_RATE_LIMIT_PER_MINUTE as MAX_PROVIDER_RATE_LIMIT_PER_MINUTE,
    MAX_PROVIDER_READ_TIMEOUT_SECONDS as MAX_PROVIDER_READ_TIMEOUT_SECONDS,
    MAX_PROVIDER_RETRIES as MAX_PROVIDER_RETRIES,
    MAX_PROVIDER_TOTAL_TIMEOUT_SECONDS as MAX_PROVIDER_TOTAL_TIMEOUT_SECONDS,
    MAX_S3_CACHE_ENTRIES as MAX_S3_CACHE_ENTRIES,
    MAX_S3_PRESETS as MAX_S3_PRESETS,
    MAX_S3_REGISTRY_ADAPTERS as MAX_S3_REGISTRY_ADAPTERS,
    MAX_SCRIPT_ACTORS as MAX_SCRIPT_ACTORS,
    MAX_SCRIPT_BEATS_PER_SCENE as MAX_SCRIPT_BEATS_PER_SCENE,
    MAX_SCRIPT_EVENTS_PER_SCENE as MAX_SCRIPT_EVENTS_PER_SCENE,
    MAX_SCRIPT_LINES_PER_SCENE as MAX_SCRIPT_LINES_PER_SCENE,
    MAX_SCRIPT_NAME_LENGTH_CHARS as MAX_SCRIPT_NAME_LENGTH_CHARS,
    MAX_SCRIPT_SCENES as MAX_SCRIPT_SCENES,
    MAX_SCRIPT_TEXT_LENGTH_CHARS as MAX_SCRIPT_TEXT_LENGTH_CHARS,
    MAX_SCRIPT_TURNS_PER_SCENE as MAX_SCRIPT_TURNS_PER_SCENE,
    MAX_STEM_RECOMBINATION_TOLERANCE_LSB_24BIT as MAX_STEM_RECOMBINATION_TOLERANCE_LSB_24BIT,
    MAX_TRUE_PEAK_DBTP as MAX_TRUE_PEAK_DBTP,
    MIN_COST_USD as MIN_COST_USD,
    MIN_DUCKING_ATTENUATION_DB as MIN_DUCKING_ATTENUATION_DB,
    MIN_DUCKING_TIME_MS as MIN_DUCKING_TIME_MS,
    MIN_GAIN_DB as MIN_GAIN_DB,
    MIN_LATENCY_RESIDUAL_SAMPLES as MIN_LATENCY_RESIDUAL_SAMPLES,
    MIN_LOUDNESS_RANGE_LU as MIN_LOUDNESS_RANGE_LU,
    MIN_LOUDNESS_TOLERANCE_LU as MIN_LOUDNESS_TOLERANCE_LU,
    MIN_NORMALIZED_LEVEL as MIN_NORMALIZED_LEVEL,
    MIN_PAN_POSITION as MIN_PAN_POSITION,
    MIN_PROSODY_PITCH_SEMITONES as MIN_PROSODY_PITCH_SEMITONES,
    MIN_PROSODY_RATE as MIN_PROSODY_RATE,
    MIN_PROSODY_VOLUME_DB as MIN_PROSODY_VOLUME_DB,
    MIN_PROVIDER_BYTES as MIN_PROVIDER_BYTES,
    MIN_PROVIDER_CONCURRENCY as MIN_PROVIDER_CONCURRENCY,
    MIN_PROVIDER_RATE_LIMIT_PER_MINUTE as MIN_PROVIDER_RATE_LIMIT_PER_MINUTE,
    MIN_PROVIDER_RETRIES as MIN_PROVIDER_RETRIES,
    MIN_RETENTION_DAYS as MIN_RETENTION_DAYS,
    MIN_SAMPLE_RATE_HZ as MIN_SAMPLE_RATE_HZ,
    MIN_STEM_RECOMBINATION_TOLERANCE_LSB_24BIT as MIN_STEM_RECOMBINATION_TOLERANCE_LSB_24BIT,
    MIN_TEXT_LENGTH_CHARS as MIN_TEXT_LENGTH_CHARS,
    MIN_TIME_SECONDS as MIN_TIME_SECONDS,
    MIN_VERSION as MIN_VERSION,
)
from kinocut_sound.validation import (
    PCM_MIX_CHANNEL_COUNTS,
    ASR_MODEL_DIGEST_RE,
    ASR_PYTHON_NAME_RE,
    ASR_VERSION_RE,
    FFMPEG_MASTER_LUFS_RANGE,
    FFMPEG_MASTER_PEAK_RANGE,
    FFMPEG_METER_VERSION_RE,
    EBUR128_SUMMARY_RE,
    ESPEAK_VERSION_RE as ESPEAK_VERSION_RE,
    SRT_CUE_SPLIT_RE as SRT_CUE_SPLIT_RE,
    SRT_TIMESTAMP_RE as SRT_TIMESTAMP_RE,
    ADAPTER_KINDS as ADAPTER_KINDS,
    ADVISORY_RE as ADVISORY_RE,
    CODE_RE as CODE_RE,
    CREATED_BY_PATTERN as CREATED_BY_PATTERN,
    DETERMINISM_CLASSES as DETERMINISM_CLASSES,
    INFORMATIONAL_FIELDS as INFORMATIONAL_FIELDS,
    ISO8601_RE as ISO8601_RE,
    LOCALE_RE as LOCALE_RE,
    RECORD_KIND_PATTERN as RECORD_KIND_PATTERN,
    REGION_RE as REGION_RE,
    SCHEME_RE as SCHEME_RE,
    SHA256_PATTERN as SHA256_PATTERN,
    TERRITORY_RE as TERRITORY_RE,
)
from kinocut_sound.capability import (
    AdapterDescriptor as AdapterDescriptor,
    AdapterLocality as AdapterLocality,
    CapabilityResult as CapabilityResult,
    CostDisclosure as CostDisclosure,
)
from kinocut_sound.consent import (
    AuditEvent as AuditEvent,
    BlendAuthorization as BlendAuthorization,
    CloudEgressGrant as CloudEgressGrant,
    ConsentGrant as ConsentGrant,
    ConsentScope as ConsentScope,
    ConsentState as ConsentState,
    RetentionPolicy as RetentionPolicy,
)
from kinocut_sound.delivery import (
    DEFAULT_PRESET as DEFAULT_PRESET,
    DeliveryPolicy as DeliveryPolicy,
    DeliveryPreset as DeliveryPreset,
    LoudnessTarget as LoudnessTarget,
    StemLayout as StemLayout,
    StemRecombinationPolicy as StemRecombinationPolicy,
)
from kinocut_sound.format import (
    CHANNEL_COUNT as CHANNEL_COUNT,
    AudioFormat as AudioFormat,
    ChannelLayout as ChannelLayout,
    ConversionPolicy as ConversionPolicy,
    DitherPolicy as DitherPolicy,
    SampleFormat as SampleFormat,
    TimeBase as TimeBase,
)
from kinocut_sound.lines import (
    Emotion as Emotion,
    Line as Line,
    ProfileRef as ProfileRef,
    PronunciationOverride as PronunciationOverride,
    Prosody as Prosody,
)
from kinocut_sound.receipt import (
    LoudnessVerification as LoudnessVerification,
    OrderedInput as OrderedInput,
    PreservationProof as PreservationProof,
    PreservationVerdict as PreservationVerdict,
    SoundReceipt as SoundReceipt,
    SoundReceiptSection as SoundReceiptSection,
    Transformation as Transformation,
)
from kinocut_sound.render_fingerprint import (
    DeterminismClass as DeterminismClass,
    FingerprintComponent as FingerprintComponent,
    RenderFingerprint as RenderFingerprint,
    ToolchainVersion as ToolchainVersion,
)
from kinocut_sound.routing import (
    AutomationEnvelope as AutomationEnvelope,
    AutomationPoint as AutomationPoint,
    Bus as Bus,
    DuckingSidechain as DuckingSidechain,
    LatencyCompensation as LatencyCompensation,
    PanLaw as PanLaw,
    Routing as Routing,
    SendReturn as SendReturn,
    Track as Track,
)
from kinocut_sound.sound_plan import (
    AssetLicenseRef as AssetLicenseRef,
    ModelRef as ModelRef,
    PlanProvenance as PlanProvenance,
    ProcessingPresetRef as ProcessingPresetRef,
    SoundPlan as SoundPlan,
)
from kinocut_sound.voice import (
    BatchPlanner as VoiceBatchPlanner,
    BatchResult as VoiceBatchResult,
    CloudTtsAdapterStub as CloudTtsAdapterStub,
    LocalSynthesisAdapter as LocalSynthesisAdapter,
    PronunciationDictionary as PronunciationDictionary,
    TtsAdapter as TtsAdapter,
    VoiceError as VoiceError,
    VoiceRoster as VoiceRoster,
    VoiceSlot as VoiceSlot,
    default_roster as default_voice_roster,
)
from kinocut_sound.post import (
    BatchClip as PostBatchClip,
    BatchPlanner as PostBatchPlanner,
    BatchResult as PostBatchResult,
    DeEssAdapter as DeEssAdapter,
    DynamicsAdapter as DynamicsAdapter,
    EqAdapter as EqAdapter,
    FFTDenoiseAdapter as FFTDenoiseAdapter,
    LoudnessAdapter as LoudnessAdapter,
    PostChain as PostChain,
    PostChainResult as PostChainResult,
    PostContext as PostContext,
    PostError as PostError,
    PostStageResult as PostStageResult,
)
from kinocut_sound.world import (
    AmbientLayer as AmbientLayer,
    AuditionContract as AuditionContract,
    CatalogAsset as CatalogAsset,
    FoleyResolver as FoleyResolver,
    LayerStack as LayerStack,
    LocationPreset as LocationPreset,
    PresetRegistry as PresetRegistry,
    SeamlessLoop as SeamlessLoop,
    WorldAssetCatalog as WorldAssetCatalog,
    WorldError as WorldError,
)
from kinocut_sound.timeline import (
    Cue as Cue,
    CueKind as CueKind,
    Timeline as Timeline,
)

__version__ = "0.1.0"

__all__ = [
    "ADAPTER_KINDS",
    "ADVISORY_RE",
    "ASR_COPY_CHUNK_BYTES",
    "ASR_MEMORY_ESTIMATE_BYTES",
    "ASR_MODEL_DIGEST_RE",
    "ASR_PYTHON_NAME_RE",
    "ASR_VERSION_RE",
    "CHANNEL_COUNT",
    "CODE_RE",
    "CREATED_BY_PATTERN",
    "DEFAULT_ADAPTER_TIMEOUT_SECONDS",
    "DEFAULT_ASR_DECODE",
    "DEFAULT_ASR_PROBE_TIMEOUT_SECONDS",
    "DEFAULT_ASR_SAMPLE_RATE_HZ",
    "DEFAULT_ASR_THREADS",
    "DEFAULT_ASR_TOTAL_TIMEOUT_SECONDS",
    "DEFAULT_BROADCAST_TRUE_PEAK_DBTP",
    "DEFAULT_BUS_GAIN_DB",
    "DEFAULT_DUB_AMPLITUDE",
    "DEFAULT_DUB_SAMPLE_RATE_HZ",
    "DEFAULT_DUB_SPATIAL_THREADS",
    "DEFAULT_DUB_TOTAL_TIMEOUT_SECONDS",
    "DEFAULT_DUB_UTTERANCE_TIMEOUT_SECONDS",
    "DEFAULT_DUB_WORDS_PER_MINUTE",
    "DEFAULT_GAP_TOLERANCE_SECONDS",
    "DEFAULT_LATENCY_RESIDUAL_SAMPLES",
    "DEFAULT_LAYER_DUCKING_ATTACK_MS",
    "DEFAULT_LAYER_DUCKING_ATTENUATION_DB",
    "DEFAULT_LAYER_DUCKING_RECOVERY_MS",
    "DEFAULT_LAYER_DUCKING_RELEASE_MS",
    "DEFAULT_LOUDNESS_DEMO_DURATION_SECONDS",
    "DEFAULT_LOUDNESS_METER_TIMEOUT_SECONDS",
    "DEFAULT_LOUDNESS_TOLERANCE_LU",
    "DEFAULT_LOUDNESS_VERSION_TIMEOUT_SECONDS",
    "DEFAULT_MASTER_LIMITER_ATTACK_MS",
    "DEFAULT_MASTER_LIMITER_RELEASE_MS",
    "DEFAULT_MASTER_LRA_LU",
    "DEFAULT_MASTER_PEAK_MARGIN_DB",
    "DEFAULT_MASTER_TOTAL_TIMEOUT_SECONDS",
    "DEFAULT_MIX_CHANNEL_COUNT",
    "DEFAULT_MIX_CONVERSION_CHEBY",
    "DEFAULT_MIX_CONVERSION_CUTOFF",
    "DEFAULT_MIX_CONVERSION_DITHER_METHOD",
    "DEFAULT_MIX_CONVERSION_PRECISION",
    "DEFAULT_MIX_CONVERSION_THREADS",
    "DEFAULT_MIX_DUCK_ATTACK_MS",
    "DEFAULT_MIX_DUCK_ATTENUATION_DB",
    "DEFAULT_MIX_DUCK_RELEASE_MS",
    "DEFAULT_MIX_DUCK_THRESHOLD",
    "DEFAULT_MIX_SAMPLE_RATE_HZ",
    "DEFAULT_PAN_POSITION",
    "DEFAULT_PRESET",
    "DEFAULT_PROSODY_EMPHASIS",
    "DEFAULT_PROSODY_PITCH_SEMITONES",
    "DEFAULT_PROSODY_RATE",
    "DEFAULT_PROSODY_VOLUME_DB",
    "DEFAULT_PROVIDER_CONNECT_TIMEOUT_SECONDS",
    "DEFAULT_PROVIDER_MAX_CONCURRENCY",
    "DEFAULT_PROVIDER_MAX_DURATION_SECONDS",
    "DEFAULT_PROVIDER_MAX_INPUT_BYTES",
    "DEFAULT_PROVIDER_MAX_OUTPUT_BYTES",
    "DEFAULT_PROVIDER_MAX_RETRIES",
    "DEFAULT_PROVIDER_RATE_LIMIT_PER_MINUTE",
    "DEFAULT_PROVIDER_READ_TIMEOUT_SECONDS",
    "DEFAULT_PROVIDER_TOTAL_TIMEOUT_SECONDS",
    "DEFAULT_PUBLIC_MIX_STEMS",
    "DEFAULT_PUBLIC_MIX_TIMEOUT_SECONDS",
    "DEFAULT_SEND_GAIN_DB",
    "DEFAULT_SEND_POST_FADER",
    "DEFAULT_SPATIAL_CROSSOVER_HZ",
    "DEFAULT_SPATIAL_DISTANCE_GAIN_DB",
    "DEFAULT_SPATIAL_DISTANCE_PCT",
    "DEFAULT_SPATIAL_DISTANCE_Q",
    "DEFAULT_SPATIAL_ROLLOFF_DB",
    "DEFAULT_STREAM_PODCAST_TRUE_PEAK_DBTP",
    "DEFAULT_TAIL_SECONDS",
    "DETERMINISM_CLASSES",
    "DUB_ASSEMBLY_MEMORY_MULTIPLIER",
    "DUB_SPATIAL_IO_BYTES",
    "DUB_SPATIAL_METADATA_BYTES",
    "DUB_SPATIAL_NATIVE_MEMORY_BYTES",
    "EBUR128_SUMMARY_RE",
    "ESPEAK_VERSION_RE",
    "FFMPEG_MASTER_LUFS_RANGE",
    "FFMPEG_MASTER_PEAK_RANGE",
    "FFMPEG_METER_VERSION_RE",
    "INFORMATIONAL_FIELDS",
    "INVALID_RECORD",
    "ISO8601_RE",
    "LOCALE_RE",
    "LOUDNESS_DIAGNOSTIC_CHUNK_BYTES",
    "MASTER_MEMORY_MULTIPLIER",
    "MAX_ADAPTER_TIMEOUT_SECONDS",
    "MAX_AMBIENT_EXTRA_REPEATS",
    "MAX_AMBIENT_LAYERS",
    "MAX_ASR_DURATION_SECONDS",
    "MAX_ASR_LAUNCHER_BYTES",
    "MAX_ASR_MODEL_BYTES",
    "MAX_ASR_OUTPUT_BYTES",
    "MAX_ASR_SAMPLE_RATE_HZ",
    "MAX_ASR_SEGMENTS",
    "MAX_ASR_STAGING_BYTES",
    "MAX_ASR_TEXT_BYTES",
    "MAX_ASR_WORDS",
    "MAX_ASSEMBLY_CLIPS",
    "MAX_ASSEMBLY_FOLEY_INTENTS",
    "MAX_ASSEMBLY_SILENCE_INTENTS",
    "MAX_ASSEMBLY_TIMELINE_CUES",
    "MAX_DUB_CAPTION_BYTES",
    "MAX_DUB_CUES",
    "MAX_DUB_CUE_CHARS",
    "MAX_DUB_MEMORY_BYTES",
    "MAX_DUB_TIMELINE_SECONDS",
    "MAX_DUB_TOTAL_CHARS",
    "MAX_DUB_WAV_BYTES",
    "MAX_DUCKING_ATTACK_MS",
    "MAX_DUCKING_ATTENUATION_DB",
    "MAX_DUCKING_RECOVERY_MS",
    "MAX_DUCKING_RELEASE_MS",
    "MAX_FINGERPRINT_ITEMS",
    "MAX_FINGERPRINT_VERSION_CHARS",
    "MAX_GAIN_DB",
    "MAX_LATENCY_RESIDUAL_SAMPLES",
    "MAX_LAYER_DUCKING_SAMPLE_VISITS",
    "MAX_LOUDNESS_DIAGNOSTIC_BYTES",
    "MAX_LOUDNESS_LUFS",
    "MAX_LOUDNESS_RANGE_LU",
    "MAX_LOUDNESS_TOLERANCE_LU",
    "MAX_MASTER_WAV_HEADER_BYTES",
    "MAX_MIX_AUTOMATION_ENVELOPES",
    "MAX_MIX_AUTOMATION_POINTS",
    "MAX_MIX_AUTOMATION_POINTS_PER_ENVELOPE",
    "MAX_MIX_AUTOMATION_WORK_UNITS",
    "MAX_MIX_CONVERSION_MANIFEST_BYTES",
    "MAX_MIX_CONVERSION_WORK_UNITS",
    "MAX_MIX_CONVERTED_BYTES",
    "MAX_MIX_DURATION_SECONDS",
    "MAX_MIX_INPUT_BYTES",
    "MAX_MIX_JSON_DEPTH",
    "MAX_MIX_LAYER_RECEIPT_BYTES",
    "MAX_MIX_MEMORY_BYTES",
    "MAX_MIX_RECEIPT_BYTES",
    "MAX_MIX_REQUEST_BYTES",
    "MAX_MIX_ROUTED_FEATURE_WORK_UNITS",
    "MAX_MIX_ROUTING_BINDINGS",
    "MAX_MIX_ROUTING_BUSES",
    "MAX_MIX_ROUTING_RECEIPT_BYTES",
    "MAX_MIX_ROUTING_TRACKS",
    "MAX_MIX_SAMPLE_RATE_HZ",
    "MAX_MIX_SENDS",
    "MAX_MIX_SEND_RECEIPT_BYTES",
    "MAX_MIX_SEND_WORK_UNITS",
    "MAX_MIX_SIDECHAINS",
    "MAX_MIX_SIDECHAIN_RECEIPT_BYTES",
    "MAX_MIX_SIDECHAIN_WORK_UNITS",
    "MAX_MIX_STEMS",
    "MAX_MIX_WORKER_DIAGNOSTIC_BYTES",
    "MAX_MIX_WORKER_MESSAGE_BYTES",
    "MAX_NORMALIZED_LEVEL",
    "MAX_PAN_POSITION",
    "MAX_PROSODY_PITCH_SEMITONES",
    "MAX_PROSODY_RATE",
    "MAX_PROSODY_VOLUME_DB",
    "MAX_PROVIDER_CONCURRENCY",
    "MAX_PROVIDER_CONNECT_TIMEOUT_SECONDS",
    "MAX_PROVIDER_DURATION_SECONDS",
    "MAX_PROVIDER_INPUT_BYTES",
    "MAX_PROVIDER_OUTPUT_BYTES",
    "MAX_PROVIDER_RATE_LIMIT_PER_MINUTE",
    "MAX_PROVIDER_READ_TIMEOUT_SECONDS",
    "MAX_PROVIDER_RETRIES",
    "MAX_PROVIDER_TOTAL_TIMEOUT_SECONDS",
    "MAX_S3_CACHE_ENTRIES",
    "MAX_S3_PRESETS",
    "MAX_S3_REGISTRY_ADAPTERS",
    "MAX_SCRIPT_ACTORS",
    "MAX_SCRIPT_BEATS_PER_SCENE",
    "MAX_SCRIPT_EVENTS_PER_SCENE",
    "MAX_SCRIPT_LINES_PER_SCENE",
    "MAX_SCRIPT_NAME_LENGTH_CHARS",
    "MAX_SCRIPT_SCENES",
    "MAX_SCRIPT_TEXT_LENGTH_CHARS",
    "MAX_SCRIPT_TURNS_PER_SCENE",
    "MAX_STEM_RECOMBINATION_TOLERANCE_LSB_24BIT",
    "MAX_TRUE_PEAK_DBTP",
    "MIN_COST_USD",
    "MIN_DUCKING_ATTENUATION_DB",
    "MIN_DUCKING_TIME_MS",
    "MIN_GAIN_DB",
    "MIN_LATENCY_RESIDUAL_SAMPLES",
    "MIN_LOUDNESS_MATERIAL_SECONDS",
    "MIN_LOUDNESS_RANGE_LU",
    "MIN_LOUDNESS_TOLERANCE_LU",
    "MIN_MEASURABLE_INTEGRATED_LUFS",
    "MIN_MIX_SAMPLE_RATE_HZ",
    "MIN_NORMALIZED_LEVEL",
    "MIN_PAN_POSITION",
    "MIN_PROSODY_PITCH_SEMITONES",
    "MIN_PROSODY_RATE",
    "MIN_PROSODY_VOLUME_DB",
    "MIN_PROVIDER_BYTES",
    "MIN_PROVIDER_CONCURRENCY",
    "MIN_PROVIDER_RATE_LIMIT_PER_MINUTE",
    "MIN_PROVIDER_RETRIES",
    "MIN_RETENTION_DAYS",
    "MIN_SAMPLE_RATE_HZ",
    "MIN_STEM_RECOMBINATION_TOLERANCE_LSB_24BIT",
    "MIN_TEXT_LENGTH_CHARS",
    "MIN_TIME_SECONDS",
    "MIN_VERSION",
    "MIX_AUTOMATION_METADATA_BYTES",
    "MIX_CONVERSION_IO_BYTES",
    "MIX_CONVERSION_METADATA_BYTES",
    "MIX_CONVERSION_NATIVE_MEMORY_BYTES",
    "MIX_CONVERSION_STARTUP_WORK_UNITS",
    "MIX_LAYER_WORK_MEMORY_MULTIPLIER",
    "MIX_ROUTING_SOURCE_MEMORY_MULTIPLIER",
    "MIX_SEND_METADATA_BYTES",
    "MIX_SIDECHAIN_METADATA_BYTES",
    "MIX_SOURCE_MEMORY_MULTIPLIER",
    "MIX_STEM_MEMORY_MULTIPLIER",
    "MIX_WORKER_IO_CHUNK_BYTES",
    "MIX_WORK_MEMORY_MULTIPLIER",
    "PCM_MIX_CHANNEL_COUNTS",
    "RECORD_KIND_PATTERN",
    "REGION_RE",
    "SCHEME_RE",
    "SHA256_PATTERN",
    "SRT_CUE_SPLIT_RE",
    "SRT_TIMESTAMP_RE",
    "TERRITORY_RE",
    "UNKNOWN_RECORD_FIELD",
    "UNSAFE_LOCATION",
    "AdapterDescriptor",
    "AdapterLocality",
    # S8 world
    "AmbientLayer",
    "AssetLicenseRef",
    "AudioFormat",
    "AuditEvent",
    "AuditionContract",
    "AutomationEnvelope",
    "AutomationPoint",
    "BlendAuthorization",
    "BoundedCode",
    "Bus",
    "CapabilityResult",
    "CatalogAsset",
    "ChannelLayout",
    "CloudEgressGrant",
    # S5 voice
    "CloudTtsAdapterStub",
    "ConsentGrant",
    "ConsentScope",
    "ConsentState",
    "ConversionPolicy",
    "CostDisclosure",
    "Cue",
    "CueKind",
    # S7 post
    "DeEssAdapter",
    "DeliveryPolicy",
    "DeliveryPreset",
    "DeterminismClass",
    "DitherPolicy",
    "DuckingSidechain",
    "DynamicsAdapter",
    "Emotion",
    "EqAdapter",
    "FFTDenoiseAdapter",
    "FingerprintComponent",
    "FoleyResolver",
    "FrozenModel",
    "LatencyCompensation",
    "LayerStack",
    "Line",
    "LocalSynthesisAdapter",
    "LocationPreset",
    "LoudnessAdapter",
    "LoudnessTarget",
    "LoudnessVerification",
    "ModelRef",
    "OrderedInput",
    "PanLaw",
    "PlanProvenance",
    "PostBatchClip",
    "PostBatchPlanner",
    "PostBatchResult",
    "PostChain",
    "PostChainResult",
    "PostContext",
    "PostError",
    "PostStageResult",
    "PreservationProof",
    "PreservationVerdict",
    "PresetRegistry",
    "ProcessingPresetRef",
    "ProfileRef",
    "PronunciationDictionary",
    "PronunciationOverride",
    "Prosody",
    "RecordBase",
    "RenderFingerprint",
    "RetentionPolicy",
    "Routing",
    "SampleFormat",
    "SeamlessLoop",
    "SendReturn",
    "Sha256",
    "SoundContractError",
    "SoundPlan",
    "SoundReceipt",
    "SoundReceiptSection",
    "StemLayout",
    "StemRecombinationPolicy",
    "TimeBase",
    "Timeline",
    "ToolchainVersion",
    "Track",
    "Transformation",
    "TtsAdapter",
    "VoiceBatchPlanner",
    "VoiceBatchResult",
    "VoiceError",
    "VoiceRoster",
    "VoiceSlot",
    "WorldAssetCatalog",
    "WorldError",
    "canonical_digest",
    "canonical_record_id",
    "contract_error",
    "default_voice_roster",
    "location_violation",
]
