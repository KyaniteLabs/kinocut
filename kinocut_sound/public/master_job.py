...
def render_master_request(payload, project_root):
    ...
    with _job(payload, project_root) as job:
        ...
        _, mode = parse_normalization(run_sync(render_args(job, measurements), job.deadline))
        data = job.accept()
...
async def render_master_request_async(payload, project_root):
    ...
    with _job(payload, project_root) as job:
        ...
        _, mode = parse_normalization(await run_async(render_args(job, measurements), job.deadline))
        data = job.accept()
