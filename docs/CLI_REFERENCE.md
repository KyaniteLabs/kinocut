...
| `sound-mix-render` | Supplied-media mix ZIP with source windows, additive ducked ambience, V2 [static routing](SOUND_ROUTING_REQUESTS.md) and V3 [ambient layers, loop fill and explicit layer ducking](SOUND_LAYER_REQUESTS.md) via `--request-json` and `--project-root`; omit both for a labelled demo. See [request contract](SOUND_MIX_REQUESTS.md). |
