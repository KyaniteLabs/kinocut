...
| `sound_mix_render` | Assemble supplied WAVs with source windows, crossfades and additive ducked ambience; V2 adds [static routing](SOUND_ROUTING_REQUESTS.md), V3 adds [ambient layers, loop fill and explicit layer ducking](SOUND_LAYER_REQUESTS.md). Takes a persisted request and project root. No-argument labelled demo remains available. See [request contract](SOUND_MIX_REQUESTS.md). |
