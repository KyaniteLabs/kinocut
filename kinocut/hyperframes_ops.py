"""Hyperframes public operations.

Thin wrappers that translate Hyperframes CLI subcommands into typed results.
The subprocess plumbing, operation schema registry, and command resolution
live in :mod:`kinocut.hyperframes_engine`; this module imports those primitives
and exposes the user-facing ``render``/``compositions``/``snapshot``/… surface.
``hyperframes_engine`` re-exports every name below so existing
