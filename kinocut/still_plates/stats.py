...
def luma_channel(arr: np.ndarray) -> np.ndarray:
    """Rec.709 luma from float RGB array."""
    return 0.2126 * arr[..., 0] + 0.7152 * arr[..., 1] + 0.0722 * arr[..., 2]


def mean_rgb(arr: np.ndarray) -> tuple[float, float, float]:
...
def apply_rgb_gains(arr: np.ndarray, gains: tuple[float, float, float]) -> np.ndarray:
...
def shared_gains_to_hero(
    ...
    *,
    ...
    for h, p in zip(hero_rgb, package_rgb, strict=True):
        ...
        gains.append(float(np.clip(g, min_gain, max_gain)))
    return gains[0], gains[1], gains[2]


def shadow_green_cyan_fraction(arr: np.ndarray) -> float:
    ...
    g = arr[..., 1]
    b = arr[..., 2]
    # Avoid div0: treat near-black as non-wash.
...
def frame_metrics(arr: np.ndarray) -> dict[str, float]:
...
def package_metrics(frames: list[np.ndarray]) -> dict[str, Any]:
