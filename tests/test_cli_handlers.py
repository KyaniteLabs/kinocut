...
class TestReleaseCheckpointPassthrough:
    """``kino release-checkpoint`` must be a pure passthrough of the MCP tool."""
    ...
    def test_handler_delegates_to_the_mcp_implementation(self, monkeypatch, capsys):
        ...
        handled = handle_advanced_commands(
            self._args(output_dir="review", min_score=70.0, frame_count=4), use_json=True
        )
    ...
    def test_handler_fails_closed_on_tool_error_result(self, monkeypatch):
        ...
        with pytest.raises(SystemExit) as excinfo:
            handle_advanced_commands(self._args(), use_json=True)
