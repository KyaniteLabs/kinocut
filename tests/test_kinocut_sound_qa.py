...
def test_asr_fake_port_is_removed():
    import kinocut_sound.qa as qa

    assert "FakeAsrPort" not in qa.__all__
    assert "verify_script_asr" not in qa.__all__
    with pytest.raises(ImportError):
        from kinocut_sound.qa.asr import FakeAsrPort  # noqa: F401
