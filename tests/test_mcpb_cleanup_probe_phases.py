...
def _run_child(
    ...
    *,
    ...
    monkeypatch.setattr(helper, "observed_pid", lambda *_args, **_kwargs: 12345)
    monkeypatch.setattr(helper, "pid_exists", lambda _pid: True)
...
def test_cleanup_state_writer_rejects_valid_enum_with_wrong_phase(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="cleanup_probe_state_sequence_invalid"):
        state.fail("launcher_started", "server_not_alive_after_launcher")
        ...
        json.dumps(
            {
                ...
                "probe_exit_class": "server_not_alive_after_launcher",
            }
        ),
        encoding="utf-8",
    )
...
def test_cleanup_child_publishes_terminal_proof_only_after_exact_server_survives(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    helper = _owner_helper()
    writes: list[str] = []
    ...
    args = _run_child(helper, tmp_path, monkeypatch)

    assert helper._read_ready(args.survival_file, args.token)
    assert [json.loads(item)["phase"] for item in writes[:-1]] == [
        "owner_activated",
        "launcher_started",
        "server_observed",
...
def test_server_observation_timeout_is_bounded(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    helper = _owner_helper()
    args = _args(tmp_path)
    launcher = _Process(stdin=_Pipe())
    monkeypatch.setattr(helper, "activate_child_from_env", lambda: "test_owner")
    monkeypatch.setattr(helper.subprocess, "Popen", lambda *_args, **_kwargs: launcher)
    monkeypatch.setattr(helper, "observed_pid", lambda *_args, **_kwargs: None)

    with pytest.raises(RuntimeError, match="server_unobserved"):
        helper.cleanup_probe_child(args, 4096, 0)

    assert helper._read_cleanup_probe_failure(args.survival_file, args.token, 4096) == {
        "failure_phase": "server_observed",
        "probe_exit_class": "server_unobserved",
    }
...
@pytest.mark.parametrize(
    ("polls", "failure_phase"),
    [([1], "stdin_keeper_started"), ([None, 1], "launcher_terminated"), ([None, None, 1], "server_alive")],
)
...
@pytest.mark.parametrize(
    ("pid_result", "expected"),
    [(False, "server_not_alive_after_launcher"), (OSError("private detail"), "server_state_unavailable")],
)
def test_exact_server_liveness_failure_is_bounded(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    pid_result: bool | Exception,
    expected: str,
) -> None:
    helper = _owner_helper()
    args = _args(tmp_path)
    launcher = _Process(stdin=_Pipe())
    keeper = _Process(polls=[None, None])
    processes = iter([launcher, keeper])
    monkeypatch.setattr(helper, "activate_child_from_env", lambda: "test_owner")
    monkeypatch.setattr(helper.subprocess, "Popen", lambda *_args, **_kwargs: next(processes))
    monkeypatch.setattr(helper, "observed_pid", lambda *_args, **_kwargs: 12345)
    ...
    def pid_exists(_pid: int) -> bool:
        if isinstance(pid_result, Exception):
            raise pid_result
        return pid_result

    monkeypatch.setattr(helper, "pid_exists", pid_exists)
    with pytest.raises(RuntimeError, match=expected):
        helper.cleanup_probe_child(args, 4096, 0.1)


def test_final_publication_failure_after_server_observation_is_bounded(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    helper = _owner_helper()
    args = _args(tmp_path)
    ...
    def fail_publication(self: Any) -> None:
        raise OSError("private detail")
